var list = require('./base');
	...
	list.insert = function (req, res, next) {
		...
		//重复添加
		D('Blog').count({email:user.email,title:title},function(err,count){
	...
	list.update = function (req, res, next) {
		...
		if('undefined'!==typeof req.xdata&&req.xdata.id!='')
		{
		  ...
		  d.updatetime = C.time();
			...
			D('Blog').update({_id:id},d,function(err,result){
			...
			else
			{
				D('Blog').delete({_id:id}, function (err) {
					if (err) {
					 ...
					 if(row.pic!=''){fs = require('fs');fs.unlink(C.static+row.pic);}
					res.redirect('/');
				});
			}
		});
